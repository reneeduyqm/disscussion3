...
class Dice:
    ...
    def __str__(self):
        return f'Last roll: {self.rolls_list[-1]}'
    ...
    def roll(self):
        new_roll = random.randint(1, self.num_sides)
        self.rolls_list.append(new_roll)
        return self.rolls_list[-1]
    ...
    def num_rolls(self):
        numrolls = input("How many times do you want to roll?")
    ...
    def print_count_for_num(self, number):
        count = 0
        for i in range(len(self.rolls_list)):

            if number == self.rolls_list[i]:
                print("Value incremented")
                count += 1
        print(str(number) + " was rolled " + str(count) + " times")
...
def main():
    ...
    # BONUS quiz
    # Print accumulation
    six_sided.print_count_for_num(5)
